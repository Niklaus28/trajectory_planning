...
def exponential_stiffness(joint_error):
    ...
    if joint_J2 < 0.03:
        ...
        #J2_stiffness = random.randint(start_range,end_range)
        J2_stiffness = 1.2
...
def joint_PMP(xyz):
    ...
    while (abs(joint_error[0]) >=0.03) or (abs(joint_error[1]) >=0.03) or (abs(joint_error[2]) >=0.03) or (abs(joint_error[3]) >=0.03):
        ...
        group.get_next_feedback(reuse_fbk=feedback)
        #joint_error = (selected_pt - feedback.position)
        previous_joint_error = joint_error
        current_joint_error = (selected_pt - feedback.position)
        for i in range(group.size):
            if abs(current_joint_error[i])> abs(previous_joint_error[i]):
                #print(current_joint_error[i]-previous_joint_error[i])
        ...
        torque_command = stiffness * joint
        ...
        group.get_next_feedback(reuse_fbk=feedback)
        new_joint_error = feedback.position
        #next_joint_error = selected_pt - feedback.position
        #for i in range(group.size):
        #    if abs(next_joint_error[i]) > abs(current_joint_error[i]):
        #        stiffness[i] = -exponential_stiffness(joint_error)[i]
