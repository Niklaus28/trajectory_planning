...
def setup(xyz_coordinate,xyz):
    ...
    home_pos = np.expand_dims(np.array([xyz.get('x'),xyz.get('y'),xyz.get('z')]),axis=-1)
    xyz_targets = np.concatenate((home_pos,xyz_coordinate),axis=1)    
...
def home_position(xyz):
    ...
    xyz_target = np.expand_dims(np.array([xyz.get('x'),xyz.get('y'),xyz.get('z')]),axis=-1)
...
def configuration(data, user_id):
    if user_id == 'User1':
        xyz = data[0].get(user_id)
    elif user_id == 'User2':
        xyz = data[1].get(user_id)
    elif user_id == 'User3':
        xyz = data[2].get(user_id)
    else:
        print("No Defined UserName Available")
...
def main():
    ...
    global xyz
    data = load_yaml()
    xyz = configuration(data, user_id)
    home_position(xyz)
    print("ExoArm main")
